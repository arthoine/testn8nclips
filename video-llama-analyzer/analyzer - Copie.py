#!/usr/bin/env python3
"""
🎥 VIDEO CLIP ANALYZER API - LLaVA-Video Edition (Windows Compatible)
...
# ⚠️ DÉSACTIVER FLASHATTENTION POUR WINDOWS (AVANT L'IMPORT)
...
if LLAVA_AVAILABLE:
    try:
        ...
        # Charger avec attn_implementation explicite pour Windows
        tokenizer, model, image_processor, max_length = load_pretrained_model(
            MODEL_PATH,
            None,
            "llava_qwen",
            torch_dtype="float16",  # Utiliser float16 au lieu de bfloat16 pour compatibilité
            device_map="auto",
            attn_implementation="eager"  # 🔧 Force eager attention (pas flash)
        )
        model.eval()
        print("✅ Modèle chargé avec succès!")
    except TypeError as e:
        # Si attn_implementation n'est pas supporté, essayer sans
        ...
        print("2. Réinstallez LLaVA-NeXT: pip uninstall llava -y && pip install git+https://github.com/LLaVA-VL/LLaVA-NeXT.git")
        print("3. Si erreur CUDA: Utilisez CPU en modifiant DEVICE = 'cpu' dans le code")
        model = None
...
def batch_analyze():
    ...
    for i, clip in enumerate(clips):
        print(f"\n📹 Analyse clip {i+1}/{total}: {clip.get('title', 'N/A')}")
